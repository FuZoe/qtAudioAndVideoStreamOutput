...
*/
    ...
    Column {
        ...
        Row {
            ...
            Column {
                ...
                Button {
                    text: "开始推流"
                    onClicked: gstStreamer.start("127.0.0.1", 5000, 1280, 720, 25)
                    width: parent.width
                }
                Button {
                    text: "停止推流"
                    onClicked: gstStreamer.stop()
                    width: parent.width
                }
    ...
    // 将所选视频源的帧推给 GStreamer
    Connections {
        target: videoOutput.videoSink
        function onVideoFrameChanged(frame) {
            if (combbox_index === 1) {
                gstStreamer.pushVideoFrame(frame)
            }
        }
    }
    Connections {
        target: videoOutput2.videoSink
        function onVideoFrameChanged(frame) {
            if (combbox_index === 2) {
                gstStreamer.pushVideoFrame(frame)
            }
